...
def read_fixed_width_mesh(path, numnp, numel):
    ...
    Supported formats:
      ...
      1. Legacy Fortran 16i5 connectivity
         followed by fixed-width coordinates.

      2. Whitespace-separated connectivity
         followed by fixed-width or whitespace-separated coordinates.

    The whitespace connectivity parser uses NumPy rather than calling
    int() on every individual token.
    """
    ...
    # --------------------------------------------------------
    # Try legacy fixed-width 16i5 connectivity.
    # --------------------------------------------------------
    ...
    if len(lines) >= nint_lines:
        ...
        try:
            for line in lines[:nint_lines]:
                ...
                # A valid 16i5 line is at most 80 characters.
                if len(line) > 80:
                    fixed_ok = False
                    break
    ...
    if fixed_ok:
        ...
        xy = read_fixed_width_coordinates(
            coord_text,
            numnp
        )
    ...
    # --------------------------------------------------------
    # Whitespace-separated connectivity.
    #
    # Find the line on which the nint-th integer occurs.
    # Then let np.fromstring do the actual conversion.
    # --------------------------------------------------------

    token_count = 0
    consumed_lines = 0
    conn_text_parts = []
    leftover_from_last_line = ""
    ...
    for line_no, line in enumerate(lines):
        ...
        remaining = nint - token_count

        if len(fields) <= remaining:

            conn_text_parts.append(stripped)
            ...
            # Connectivity ends in the middle of this line.
            conn_text_parts.append(
                " ".join(fields[:remaining])
            )

            leftover_from_last_line = " ".join(
                fields[remaining:]
            )
    ...
    conn_text = "\n".join(conn_text_parts)

    ien_vals = np.fromstring(
        conn_text,
        sep=" ",
        dtype=np.int64,
    )

    if len(ien_vals) != nint:
        raise ValueError(
            f"{path}: expected {nint} connectivity values after parsing, "
            f"got {len(ien_vals)}"
        )
    ...
    # Coordinates begin after the connectivity section.
    remaining_text = "".join(
        lines[consumed_lines:]
    )

    if leftover_from_last_line:
        remaining_text = (
            leftover_from_last_line
            + "\n"
            + remaining_text
