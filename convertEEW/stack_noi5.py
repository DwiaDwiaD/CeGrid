...
def read_fixed_width_mesh(path, numnp, numel):
    """
    Read mesh.data / mesh1.data without imposing the old Fortran i5
    connectivity limitation.

    Supported connectivity formats:
      1. Legacy Fortran 16i5
      2. Whitespace-separated integers (preferred)

    Coordinates are read after the connectivity section. The usual
    fixed-width 5e16.9 coordinate format is supported, with a whitespace
    fallback.

    A file whose 6-digit node IDs have already overflowed a genuine i5
    field cannot be recovered reliably because its field boundaries are
    ambiguous. Such a mesh must be regenerated with whitespace-separated
    or wider integer fields.
    """
    ...
    # --------------------------------------------------------
    # First try the legacy 16i5 format.
    # --------------------------------------------------------
    ...
    if len(lines) >= nint_lines:
        try:
            for line in lines[:nint_lines]:
                ...
                for i in range(0, min(len(line), 80), 5):
                    field = line[i:i + 5]
                    if field.strip():
                        value = int(field)

                        # A true i5 value cannot exceed this range.
    ...
    if fixed_ok:
        ...
        # If the integer section was valid but coordinates weren't in the
        # exact fixed-width layout, accept whitespace-separated coordinates.
        ...
        if len(coord_vals) == nreal:
            return ien, coord_vals[1:].reshape(numnp, 2)

        raise ValueError(
            f"{path}: could not parse the coordinate section"
        )

    # --------------------------------------------------------
    # Fall back to whitespace-separated connectivity.
    # --------------------------------------------------------
    ...
    for line_no, line in enumerate(lines):
        ...
        if not fields:
            consumed_lines = line_no + 1
            continue

        for field in fields:
            if len(conn_vals) == nint:
                break

            try:
                conn_vals.append(int(field))
            except ValueError as exc:
                raise ValueError(
                    f"{path}: encountered non-integer token {field!r} "
                    f"before reading all {nint} connectivity integers. "
                    "Regenerate the mesh if its i5 fields overflowed."
                ) from exc
    ...
    if len(conn_vals) != nint:
        raise ValueError(
            f"{path}: expected {nint} connectivity integers, "
            f"got {len(conn_vals)}"
        )
    ...
    if len(coord_vals) != nreal:
        raise ValueError(
            f"{path}: expected {nreal} coordinate values, "
...
def main():
    ...
    # fort.1 is compulsory, matching the original x_3d().
